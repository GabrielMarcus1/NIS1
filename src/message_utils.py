...
def create_message_data():
    ...
    metadata = {"Caption": caption, "Image": data_b64}

    message = {"Filename": filename, "Timestamp": timestamp, "Data": metadata}
    return message
...
def generate_confidentiality(secret_key, message, public_key):
    ...
    data = {"session_key": rsa_encryption, "encrypted_file": aes_encryption}
...
def decrypt_message_PGP_image(message, private_key):
    ...
    if verify_digest == True:
        ...
        print(
            "The message has not been verified and the hash is incorrect. Message may have been tampered with"
        )
...
def create_image_message(message, caption):
    ...
    metadata = {"Caption": "PLEASE SUPPLY A CAPTION", "Image": data_b64}
